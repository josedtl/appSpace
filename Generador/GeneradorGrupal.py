...
from GeneradorGrupal import CamposEntity
from GeneradorGrupal import TablaEntity
class TablaEntity:
    Name:str
    Detalle :List[CamposEntity]
...
class DatabaseApp:
    ...
    def __init__(self, root):
        ...
        self.refresh_button = tk.Button(root, text="Aceptar", command=self.refresh_databases)
        ...
        self.tables_listbox = tk.Listbox(self.tables_frame, selectmode=tk.MULTIPLE, width=40, height=5)
        self.tables_listbox.grid(row=1, columnspan=2, pady=5)
    ...
    def compile_action(self):
        ...
        Cabecera = TablaEntity() 
        for indice in selected_tables:
            Cabecera = TablaEntity() 
            item = self.tables_listbox.get(indice)
            Cabecera.Name =item
            self.ListaTabla.append(Cabecera)

        print(self.ListaTabla)
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(f"DESCRIBE {selected_tables}")
    ...
    def show_tables(self, event):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                for database in table_names:
                    self.tables_listbox.insert(tk.END, database)
